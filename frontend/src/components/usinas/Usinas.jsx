...
const headerProps = {
    icon: 'usinas',
    title: 'Usinas',
    subtitle: 'Gráfico de produção da Usina!'
}

const baseUrl = 'http://localhost:3001/grandeza'
const initialState = {
    usina: { grandeza_tensao:''},
    grandeza: { nome:'Produção', variavel: "potencia_kW"},
    list: []
}
...
export default class ClienteCrud extends Component {
    ...
    updateField(event) {
        const usina = { ...this.state.usina }
        usina[event.target.name] = event.target.value
        this.setState({ usina })
    }
...
load(grandeza){
    ...
    renderRows() {
      ...
      const data = [
        ...
        return this.state.list.map( grandeza => {

            const mediaVariavel = grandeza.variavel

            const somaValores = parseFloat( data.reduce( (prevValue, objeto) => prevValue + objeto[mediaVariavel], 0)/data.length).toFixed(2)
                       
            return (
                
                <tr key={grandeza.grandeza_id}>
                  
                    <td>{grandeza.nome}</td>

                    <td></td>
                                     
                    <td>{somaValores}   {grandeza.simbolo}                    
    ...
    render() {        
        ...
        const vari = this.state.grandeza.variavel       
        ...
        const data = [
          ...
          this.valor_media = this.state.mediaVariavel
            ...
            <Main {...headerProps}>
                ...
                {this.renderForm()}
                    ...
                    <LineChart width={1000} height={300} data={data}>
                        ...
                        <Line type="monotone" dataKey={vari} stroke="#8884d8" />
